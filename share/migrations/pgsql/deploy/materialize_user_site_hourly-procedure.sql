...
BEGIN
   INSERT INTO user_site_hourly(local_time,remote_user,site,total_time,total_bytes, tag_id)
      SELECT
         date_trunc('hour', ltime) AS local_time,
         COALESCE(ruser, HOST(ip)) AS remote_user,
         site AS site,
      ...
      FROM
         access_log
      WHERE
         date_trunc('day', ltime) = relevant_day
         AND code <> 'TCP_DENIED'
         AND code LIKE 'TCP_%'
      GROUP BY
         local_time,
